...
- **Inferência (Bloco 02):** TCL(ok), distribuições t e qui-quadrado(ok), intervalo de confiança(ok), teste de hipótese(ok), erros tipo I e II(ok), p-valor, poder do teste.
- **Correlação, regressão e amostragem (Bloco 03):** Pearson e Spearman(ok), regressão linear simples e múltipla, mínimos quadrados, R² e R² ajustado, resíduos, heterocedasticidade, multicolinearidade, regressão logística, técnicas de amostragem.
